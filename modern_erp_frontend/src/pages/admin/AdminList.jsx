...
const AdminList = () => {
  ...
  // Fetch admins with search term after user stops typing
  useEffect(() => {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const response = await api.get('/api/admin/getAll', { params });
      ...
      setAdmins(Array.isArray(content) ? content : []);
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow">
      ...
      {/* User Table (only show if there are admins) */}
      {admins.length > 0 && (
        <UserTable 
          users={admins} 
          userType="admin"
